...
export default function PostsList(props: IPostsListProps) {
      ...
      if (pagination.postsOnScreen.length === 0 && pagination.allPosts) {
        setPagination((lastPagination) => ({
          ...
          postsOnScreen: [
            ...getPostsOnScreen(
              0,
              lastPagination.maxPostsPerPage,
  ...
  useEffect(() => {
    if (router.asPath !== router.route) {
      const currentPageQuery = router.query.currentPage
        ? parseInt(router.query.currentPage as string)
        : pagination.currentPage;
      if (currentPageQuery !== pagination.currentPage) {
        const maxPages = Math.floor(
          pagination.allPosts.length / pagination.maxPostsPerPage
        );
        if (currentPageQuery >= 0 && currentPageQuery <= maxPages) {
          if (pagination.allPosts.length > 0) {
            if (
              pagination.maxPostsPerPage > -1 &&
              currentPageQuery !== pagination.currentPage
            ) {
              const newPostsOnScreen = getPostsOnScreen(
                currentPageQuery,
                pagination.maxPostsPerPage,
                pagination.allPosts
              );
              setPagination((lastPagination) => ({
                ...lastPagination,
                postsOnScreen: [...newPostsOnScreen],
