...
export const getStaticProps: GetStaticProps<IBlogPageProps, IParams> = async (
  context
) => {
  const params = context.params as IParams;
  const allSeriaziablePosts = await serverUtils.getSeriaziablePosts(
    "./_content/_posts"
  );
...
export default function PostsPage(props: IBlogPageProps) {
  ...
  const allPosts = props.allSeriaziablePosts
    ? props.allSeriaziablePosts.map((seriaziablePost) =>
        utils.seriaziablePostToPost(seriaziablePost)
      )
    : [];
  const slug = props.slug && props.slug.length > 0 ? props.slug : [];
  const singleView =
    slug && _.isString(slug[0])
      ? ["category", "tag", "date"].indexOf(slug[0]) === -1
      : true;

  if (!matchingPosts) {
    const sortedPosts = utils.getPostsByDate(
      utils.getPostsMatchingQuery(allPosts, slug)
    );
  ...
  const listTitleParagraph =
    listTitle.length > 0 ? (
      <h1 style={{ marginLeft: "calc(.9em)" }}>{listTitle}</h1>
    ) : null;
  ...
  return (
    <Fragment>
      ...
      {listTitleParagraph}
      <BlogBody singleView={singleView} posts={matchingPosts} />
    </Fragment>
  );
}
